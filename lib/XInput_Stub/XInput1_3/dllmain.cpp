...
bool WINAPI DllMain(HMODULE hModule, DWORD dwReason, LPVOID lpReserved)
{
    ...
    case DLL_PROCESS_ATTACH:
        ...
        // Load dll
        char path[MAX_PATH];
        if (GetFileAttributesA("xinput1_3.Chained.dll") != INVALID_FILE_ATTRIBUTES)
        {
            // If xinput1_3.Chained.dll exists, load it
            strcpy_s(path, "xinput1_3.Chained.dll");
        }
        else
        {
            // Load system xinput1_3.dll
        ...
        {
            //Log() << "Failed to load xinput1_3 library!";
        }
